<?php
class CutModel extends CI_Model 
{
    ...
    {   
        $this->db->select('t1.*,t2.party_name,t3.item_name,t5.username as user_name'); 
    ...
    {   
        $query=$this->db->select('t1.*,t2.party_name,t3.item_name,t5.username as user_name')->from('cut as t1')
                ...
                ->group_start()
                        ...
                        ->or_like('t1.total_purchase_mtr',$search)
                        ->or_like('t1.total_fent',$search)
                ->group_end()
                ->limit($limit,$start)
                ->order_by($col,$dir)
                ->get();
    ...
    {
        $query=$this->db->select('challan_no')->from('cut')
                ->order_by('id_cut',"DESC")
                ->limit('1')
                ->get();
