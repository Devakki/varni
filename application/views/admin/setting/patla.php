<div class="content">
    <div class="container-fluid">
            ...
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <ol class="breadcrumb float-right">
                        ...
                        <li class="breadcrumb-item"><a href="<?php echo base_url('Patla/index');?>"><?php echo $page_title; ?></a></li>
                    </ol>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card-box">
                    <center><h4 class="header-title m-t-0"><?php echo (($method=="edit")?"Update Patla":"Add Patla" )?></h4></center><br>
                    <form id="<?php echo (($method=="edit")?"$frm_id":"$frm_id");?>" class="form-horizontal" role="form" >
                          <div class="form-group row">
                              <label for="name" class="col-3 col-form-label">PATLA NAME<span class="text-danger">*</span></label>
                              <div class="col-9">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                ...
                                <?php echo (($method=="edit")?'<input type="hidden" name="id" value="'.$result->patla_id.'">':""); ?>
                            </div>
                        </div>
                        <?php endif; ?>
    ...
    $(document).ready(function() {
      ...
      <?php if($method !="edit") :?>
        var table = $('#datatable-buttons').DataTable({
            ...
            order: [],
            ajax: {
                   "url": "<?php echo base_url('Patla/getLists/'); ?>",
                   "type": "POST"
               },
            "order": [[1, "asc" ]],
            columns: [      
      ...
      <?php endif; ?>
       $('#datatable-buttons').on('click', '[data-id=delete]', function () {                        
            ...
            swal({
                ...
                $.ajax({
                  type: "POST",
                  url: "<?php echo base_url('Patla/delete/');?>"+id+"",
        ...
        $("#Add_frm").submit(function(event){
           ...
           $.ajax({
               url: "<?php echo base_url('Patla/create')?>",
               type: "POST",
               data: data,              
        ...
        $("#Edit_frm").submit(function(event){
           ...
           var data=$(this).serialize();            
           $.ajax({
               url: "<?php echo base_url('Patla/update')?>",
               type: "POST",
               data: data,              
               success: function(result){
                   var result  = JSON.parse(result);
                   if(result.status=="success"){
                       swal("success",result.msg,"success","#4fa7f3").then(function () { 
                        window.location="<?php echo base_url("Patla");?>";            
                   });
                   }else{
                       swal("error",result.msg,"warning","#4fa7f3");
                   }
